...
function App() {
  ...
  <Block width='w-2/5'>
    <div className='flex justify-between mb-3'>
      ...
      <button
        className='shadow-indigo-500/50 shadow-2xl rounded-lg bg-indigo-500 px-5 py-3 text-xl font-bold text-white hover:bg-indigo-600 focus:outline-none active:bg-indigo-800'
    ...
    {decks.map((deck) => (
      <div key={deck.id} onClick={(e) => {e.preventDefault(); handleButtonClick('study', deck.id)(e)}} className='flex justify-around group/edit mb-7 hover:bg-slate-700 ring group/item rounded-lg bg-gray-600'>
        
          <h2 className='text-xl font-bold text-white'>
            {deck.name}
          </h2>
          <div className='invisible group-hover/item:visible' onClick={(e) => {e.stopPropagation()(e)}}>


            <div className='z-20'>
              <button className='hover:rotate-90 transition duration-500' onClick={handleButtonClick('settings')}>
              ...
              {settingsOpen && (
                <div className='flex flex-col'>
                  <button className='' onClick={() => handleButtonClick('edit', deck.id)}>
                    Edit
                  </button>
                  <button className='' onClick={() => handleButtonClick('delete', deck.id)}>
                    Delete
                  </button>
                </div>
             ...
             {/*
               <button
                className='shadow-gray-500/50 shadow-2xl rounded-lg bg-gray-500 px-5 py-2 text-xl font-bold text-white hover:bg-gray-600 focus:outline-none active:bg-gray-800'
                onClick={() => handleButtonClick('edit', deck.id)}
              >
                Edit
              </button>
              <button
                className='shadow-red-500/50 shadow-2xl rounded-lg bg-red-500 px-5 py-2 text-xl font-bold text-white hover:bg-red-600 focus:outline-none active:bg-red-800'
                onClick={() => handleButtonClick('delete', deck.id)}
              >
                Delete
              </button>
              */}
